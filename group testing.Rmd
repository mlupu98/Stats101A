...
ggplot(data = test_data, aes(x = newPos, y = WageNew)) + geom_point()
?plot
#plot(predi)
```
...
# use for function to get average for each team

length(player_data$Club) # 12745

for(i in 1:8){
  speci_team <- player_data[which(player_data$Club == player_data$Club[i]),]
  team_name <- speci_team$Club[1]
  x_values <- na.omit(speci_team$Overall)
  x_bar <- sum(x_values)/length(x_values)
  avg_team <- c(x_bar, team_name)
  player_data[which(player_data$Club == player_data$Club[i]),]$Team_average <- x_bar
  
}


 player_data$Team_average
