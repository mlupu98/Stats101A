...
tr_func <- function(train_ratio, y_power = 1, response, ...) {
    ...
    for(i in 1:10) {
        ...
        lin_reg <- lm(response[train_rows] ~ 0 + predictors[[1]][train_rows] + predictors[[3]][train_rows] + predictors[[4]][train_rows]) #+ predictors[[5]][train_rows])
...
tr_func(0.8, 1, player_data$WageNew, player_data$Overall, player_data$Potential, player_data$HL, player_data$International.Reputation, player_data$Club )
